---
title: "Session 3 (Data Visualization With `ggplot2`): Exercise"
format: html
editor: source
---

**If you make changes to this file, please save it as `session_3_exercise_nn.qmd`, where "nn" are your initials. This is so you'll have your own copy with your changes, and you'll still have access to the original if you need it.**
...
```{r}
ggplot(______) +
  _____(mapping = aes(_______),
                 binwidth= ______)    

# Or, instead of binwidth = .... , try bins = ....!
...
-   In `ggplot2`, the word "color" refers to 1D **lines**, and the word "fill" refers to 2D **shapes**. If you are trying to color something and it's not working, it might be that what you need to do is replace the word "color" with "fill"!
...
**Your turn 4:** Notice where we've put "fill" -- it's inside the parentheses for `geom_histogram` but it's outside the parentheses for `aes`. Then, add a value for fill. Try "purple" and also try "#AA00CC". Heck, try other colors while you're at it!
...
**Your turn 6:** Run the chunk below!
...
**Your turn 8:** Try running the code below. This time there's nothing you have to add or fill in!
...
**Your turn 9:** Try running the code below. This time there's nothing you have to add or fill in! If you want, however, you can try different values between 0 and 1 for alpha, to understand how alpha works.
...
**Your turn 10:** Copy and paste the previous code chunk above. Remove the `alpha` argument and set the `position` argument to "dodge". Make sure the last argument in `geom_histogram` isn't followed by a comma, or R will wonder what else you meant to add! Then run the chunk.
...
**Your turn 11:** Try running the code below. This time there's nothing you have to add or fill in! Do notice, however the new last line we added: `facet_wrap`.
...
Of course, you want your labels to be useful. Below, we added some sample better labels for the x and y axes, as well as a title and subtitle.

**Your turn 12:** Run the chunk below. Then, change the title and subtitle to suit what you think is best (or remove the subtitle if you prefer). Feel free to alter the axis labels as well.
...
There are other kinds of plots that we won't get into much today. We'd rather go deep on just one plot rather than broad across many types of plots. But still, it's worth taking a quick look at some examples. We're adding a few details here we didn't get to teach formally, so you can play with them.

**Your turn 13:** Run the four chunks below to see some other examples!
...
ggplot(data = cirrhosis_study) +
  geom_boxplot(mapping = aes(x = Stage,
                             y = Bilirubin, 
                             group = Stage)) +
  labs(title = "Median Bilirubin Increases\nAs Liver Disease Advances",
       x = "Disease Stage") +
  theme_dark()
```

Example 2: Show a scatterplot of copper (on the x axis) and bilirubin (on the y axis) measurements, colored by the sex of the subject. On top of that, add an additional plot, the linear model of the relationship of these two variables.

```{r}
ggplot(data = cirrhosis_study) +
  geom_point(mapping = aes(x = Copper,
                           y = Bilirubin, 
                           color = Sex)) +
  geom_smooth(mapping = aes(x = Copper,
                           y = Bilirubin),
              method = "lm") +
  scale_color_manual(name = "Participant Sex",
                     values = c("F" = "#ED1F7F",
                                "M" = "#41B6E6"))
